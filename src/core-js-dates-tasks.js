...
function getNextFriday(date) {
  ...
  const day = 5;
  if (weekDay < day) {
    newDay = (day - weekDay) * 86400000;
  } else if (weekDay === day) {
    newDay = 7 * 86400000;
  } else {
    newDay = 6 * 86400000;
  }
  return new Date(Date.parse(date) + newDay);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    (Date.parse(new Date(dateEnd)) -
      Date.parse(new Date(dateStart)) +
      86400000) /
    86400000
  );
}
...
function getCountWeekendsInMonth(month, year) {
  const fullDays = new Date(year, month, 0).getDate();
  const firstDay = new Date(year, month - 1, 1).getDay();
  if (fullDays === 31) {
    if (firstDay === 0 || firstDay === 4) {
      return 9;
    }
    if (firstDay === 6 || firstDay === 5) {
      return 10;
    }
  }
  if (fullDays === 30) {
...
function getWeekNumberByDate(date) {
  ...
  const days = (Date.parse(date) - Date.parse(new Date(year, 0))) / 86400000;
  if (firstDay === 0) {
